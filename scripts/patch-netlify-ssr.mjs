 ...
 * Supports both Netlify, Vercel and generic Nitro outputs.
 *
...
const candidatePaths = [
  resolve(projectRoot, ".netlify/functions-internal/server/_ssr/ssr.mjs"),
  resolve(projectRoot, ".vercel/output/functions/__server.func/_ssr/ssr.mjs"),
  resolve(projectRoot, ".output/server/_ssr/ssr.mjs"),
];

let patchedCount = 0;

for (const ssrMjsPath of candidatePaths) {
  if (!existsSync(ssrMjsPath)) {
    continue;
  }

  console.log(`[patch-ssr] Found ssr.mjs at: ${ssrMjsPath}`);
  ...
  if (content.includes(wrongNamespaceImport)) {
    ...
    console.log("[patch-ssr] Removed previously-added wrong namespace import.");
  }
  ...
  if (hasSsrExportsBug) {
    ...
    if (!ssr2ImportMatch) {
      console.error("[patch-ssr] BUG1: Could not find ssr2.mjs named import. Skipping file.");
      continue;
    }
    ...
    changed = true;
    console.log("[patch-ssr] ✓ BUG1 fixed: added 'const ssr_exports = { default: server_default }'");
  } else {
    console.log("[patch-ssr] BUG1: No patch needed (ssr_exports already defined).");
  }
  ...
  if (content.includes(createMiddlewareArrow)) {
    ...
    if (closingIdx !== -1) {
      ...
      changed = true;
      console.log("[patch-ssr] ✓ BUG2 fixed: createMiddleware converted to function declaration");
    } else {
      console.warn("[patch-ssr] BUG2: Could not find closing }; for createMiddleware.");
    }
  } else if (content.includes(createMiddlewareFunc)) {
    console.log("[patch-ssr] BUG2: No patch needed (createMiddleware is already a function).");
  } else {
    console.log("[patch-ssr] BUG2: createMiddleware not found, skipping.");
  }

  if (changed) {
    writeFileSync(ssrMjsPath, content, "utf-8");
    console.log(`[patch-ssr] ✓ Patched ${ssrMjsPath} written to disk.`);
    patchedCount++;
  } else {
    console.log("[patch-ssr] No changes needed for this file.");
  }
}

if (patchedCount === 0) {
  console.log("[patch-ssr] Finished: no unpatched files were found.");
} else {
  console.log(`[patch-ssr] Finished: successfully patched ${patchedCount} file(s).`);
}
